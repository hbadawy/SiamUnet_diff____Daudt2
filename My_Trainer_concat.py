...
if __name__ == "__main__":
    ...
    # class_weights = torch.tensor([0.7]).to(device)
    # loss_function = torch.nn.BCELoss(weight=class_weights)

    # pos_weight = torch.tensor([2.0]).to(device)   #positive class weight
    # loss_function = nn.BCEWithLogitsLoss(pos_weight=pos_weight)
    # loss_function = torch.nn.BCELoss()

    # loss_function = FocalLoss2()
    # loss_function = DiceLoss2()
    loss_function = FocalTverskyLoss()


    num_epochs = 20
    save_path = "E://VS Projects//test_25-2-2025_FC//Training_Concat//checkpoint5-FocalTverskyLoss-20epochs"
